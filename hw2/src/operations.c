           ...
           case MENU_MATCH :
             switch (menuval) {
               ...
               case O_ABORT :
                   if (j>0){
                       for (int i = 0; i <j ; i++) {
                           if (otherdata[i]!=NULL&&strlen(otherdata[i])!=0){
                               free(otherdata[i]);
                           }
                       }
                   }
                     for (int l = 0; l < N_BASIC_FIELDS; ++l) {
                         if (basicdata[l]!=NULL&&strlen(basicdata[l])!=0){
                             free(basicdata[l]);
                         }
                     }
                     return;
