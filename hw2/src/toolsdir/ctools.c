...
void string_upcase (astring) char *astring;
{
  while (*astring) {
...
void string_downcase (astring) char *astring;
{
  while (*astring) {
...

{
  ...
  while (T) {
    ...
    switch (len = getNewLine(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int getNewLine (iport,buffer,buflen) FILE *iport; char *buffer; int buflen;
...

{
  int len;
  char *line;
  if (0 > (len = getNewLine(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...
{  
  ...
  while (T) {
    ...
    if (0 > (len = getNewLine(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
