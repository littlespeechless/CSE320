...

{
  ...
  if(fclose(tempfp)||fclose(copyfp)){
      fprintf(stderr,"Fail Saved File Due to space limitation!");
      roloexit(-1);
  }
//  fclose(tempfp);
//  fclose(copyfp);
...
//    }
    ...
    while ((opt = getopt(argc,argv,string))!=-1){
        switch (opt){
            case 'l':
                if (L_FLAG==1){
                    fprintf(stderr,"Illegal syntax option\nusage: %s\n",USAGE);
                    roloexit(-1);
                    break;
                }
            ...
            case 's':
                ...
                // if (L_FLAG!=1){
                //     fprintf(stderr,"Illegal syntax using -s option\nusage: %s\n",USAGE);
                //     roloexit(-1);
                // } else{
                //     S_FLAG =1;
                // }
            ...
            case 'u':
                ...
                if (optarg==NULL){
                    ...
                    strcpy(rolodir,home_directory(optarg));
                    if (*rolodir == '\0') {
